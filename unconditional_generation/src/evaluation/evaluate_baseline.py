...
from src.utils.scoring_func import (compute_druglike_properties, 
                                    ...
                                    get_rdkit_rmsd_tfd)
from src.utils.misc import load_pickle, get_logger


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--sdf_folder_path', type=str, help='Path to the folder containing generated SDF files')
    parser.add_argument('--num_samples', type=int, default=10000, help='Number of samples to evaluate')
    ...
    # Set up logger
    logger = get_logger('evaluate_baseline', args.sdf_folder_path)
    logger.info('---------------------------------------------')
    ...
    for file in sorted(os.listdir(args.sdf_folder_path)):
        metrics_df = pd.DataFrame()
        if not file.endswith(".sdf"):
            continue
        logger.info('*****************************************')
        sdf_path = os.path.join(args.sdf_folder_path, file)
        logger.info(f'Evaluating {sdf_path}...\n')

        # load generated molecules
        mols = [mol for mol in SDMolSupplier(sdf_path) if mol is not None]
        logger.info(f'Loaded {len(mols)} generated molecules\n')

        # Compute basic metrics
        logger.info('Computing basic metrics...')
        basic_metrics = compute_basic_metrics_baseline(mols, 
        ...
        if args.posebusters and not os.path.exists(f'{args.sdf_folder_path}/{file.rstrip(".sdf")}_posebusters.csv'):
            logger.info('Computing PoseBusters metrics...')
            pb_df = compute_posebusters_parallel(sdf_path, 
                                        save_path=f'{args.sdf_folder_path}/{file.rstrip(".sdf")}_posebusters.csv')
            logger.info(f'PoseBusters metrics saved to {args.sdf_folder_path}/{file.rstrip(".sdf")}_posebusters.csv')
        else:
            pb_df = None
            logger.info('PoseBusters metrics computation skipped.')

        # Compute druglike properties
        if args.druglike and not os.path.exists(f'{args.sdf_folder_path}/{file.rstrip(".sdf")}_druglike.csv'):
            logger.info('Computing druglike properties...')
            druglike_metrics = compute_druglike_properties(mols, save_path=f'{args.sdf_folder_path}/{file.rstrip(".sdf")}_druglike.csv')
            logger.info(f'Druglike properties saved to {args.sdf_folder_path}/{file.rstrip(".sdf")}_druglike.csv')
        else:
        ...
        if args.rdkit_rmsd_tfd:
            ...
            rmsd_tfd_metric = get_rdkit_rmsd_tfd(mols, save_path=f'{args.sdf_folder_path}/{file.rstrip(".sdf")}_rdkit_rmsd_tfd.pkl')
        ...
        if os.path.exists(f'{args.sdf_folder_path}/{file.rstrip(".sdf")}_posebusters.csv'):
            logger.info('Loading PoseBusters metrics...')
            pb_df = pd.read_csv(f'{args.sdf_folder_path}/{file.rstrip(".sdf")}_posebusters.csv')
            pb_summary = get_posebusters_summary(pb_df, num_samples=args.num_samples)
            logger.info(pb_summary)
        if os.path.exists(f'{args.sdf_folder_path}/{file.rstrip(".sdf")}_druglike.csv'):
            logger.info('Loading druglike properties...')
            druglike_metrics = pd.read_csv(f'{args.sdf_folder_path}/{file.rstrip(".sdf")}_druglike.csv')
    ...
    final_metrics_df.to_csv(os.path.join(args.sdf_folder_path, 'final_metrics_summary.csv'))
    logger.info(f"Final metrics summary saved to {args.sdf_folder_path}/final_metrics_summary.csv")
    logger.info('----------------------------------------')
